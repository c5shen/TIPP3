...
'''
def writeAbundance(refpkg, query_placement_paths, pool, lock):
    ...
    support_value = '0.95'
    try:
        support_value = getattr(Configs, Configs.placement_method).support_value
    except (AttributeError, ValueError) as e:
        pass
    _LOG.info(f"Using support value={support_value}")
    for marker, classification_path in classification_paths.items():
        ...
        filtered_path = os.path.join(clas_outdir,
                f"placement.classification.{support_value}")
